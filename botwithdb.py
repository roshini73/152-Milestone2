...
class ModBot(discord.Client):
    ...
    async def handle_channel_message(self, message):
        ...
        if num_reports <= 0:
            ...
            await mod_channel.send(f'New entry for user: {message.author.name} added to db')
        ...
        # this user has had 5 or less reports filed against them
            ...
            if score <= 5:
                await mod_channel.send(f'Entry for user: {message.author.name} updated')
            elif score > 5:
                await mod_channel.send(f'User: {message.author.name} has had more than 5 reports and will be kicked out of the group')
                await message.channel.send(f'User: {message.author.name} has had more than 5 reports and is kicked out of the group')
